...
void CAimbot::Main(ValveSDK::CUserCmd* pUserCmd, CBaseEntity* pLocal, ValveSDK::CBaseCombatWeapon *pWeapon)
{
...
void CAimbot::Main_RCS(ValveSDK::CUserCmd* pUserCmd, CBaseEntity* pLocal, ValveSDK::CBaseCombatWeapon *pWeapon)
{
...
bool CAimbot::Main_Triggerbot(Vector vSource, Vector qCurAngle, Vector vForward, Vector vRight, Vector vUp, float fMaxDistance, int iLocalTeamNumber, ValveSDK::CBaseCombatWeapon *pWeapon, CBaseEntity *pLocal, int iSeed)
{
	...
	{
		bool m_RifleSilenced = *(bool*)((DWORD)pWeapon + (DWORD)0x780);
		bool m_PistolSilenced = *(bool*)((DWORD)pWeapon + (DWORD)0x778);

		auto spread = pWeapon->GetSpread(pLocal->GetFlags(), pLocal->GetFOV(), pWeapon->GetCone(), m_RifleSilenced, m_PistolSilenced, pLocal->GetVelocity(), pWeapon->GetWeaponID());
		...
		float random[2] =
		{
			g_NoSpread.RandomFloat(-0.5f, 0.5f) + g_NoSpread.RandomFloat(-0.5f, 0.5f),
			g_NoSpread.RandomFloat(-0.5f, 0.5f) + g_NoSpread.RandomFloat(-0.5f, 0.5f),
		};
	...
	if (tr.m_pEnt && tr.m_pEnt->isValidPlayer())
	{
		//kolonote:
...
bool CAimbot::Triggerbot(ValveSDK::CBaseCombatWeapon *pWeapon, CBaseEntity *pLocal, ValveSDK::CUserCmd* pUserCmd)
{
	Vector vEyePosition = pLocal->GetEyePosition();
